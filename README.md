...
- `PACKED`: one ordered `item_observations` entry per manifest item with exact `item_id`, `item_status`, `quantity_status`, `substitution_index`, and manifest-ordered `condition_statuses`.
- `PICKED_UP`: exact `pickup_observation` enum.
- `DELIVERED`: exact `delivery_observation` enum.
- `CUSTOMER_CLAIM`: exact `item_id`, `claim_category`, `criterion_kind`, and manifest-bounded `criterion_index`.
...
The `order-fg-batch-demo-*.json` family is the fixed three-record recovery example. Its committed history is ordered as `PACKED`, `PICKED_UP`, `DELIVERED`, then one customer claim for each of the manifest's three distinct items at indices `3`, `4`, and `5`. The cure replaces exactly those three claims, is canonical UTF-8 with one LF, and pins digest `0x41548c1306fd12785adc5dd6400b2ac235849777ba9de31dea4e7f367239587c`. Tests derive every cure actor/action/item slot from the committed target records and cross-check them against the three-item manifest. The browser scenario separately proves locally that three stale customer claims produce one `submit_cure_evidence` wallet request and a fresh active set `0, 1, 2, 6`; it is deterministic local evidence, not a StudioNet transaction or deployment claim.
